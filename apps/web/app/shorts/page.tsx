...
export default function ShortsPage() {
    ...
    async function load() {
      ...
      if (user) {
        ...
        const result = selectedCategory
          ? await getUserVideosByCategoryAndFormat(user.uid, selectedCategory, "short", 20)
          : await getUserVideosByFormat(user.uid, "short", 20);
        ...
        const [result, cats] = await Promise.all([
          selectedCategory
            ? getVideosByCategoryAndFormat(selectedCategory, "short", 20)
            : getVideosByFormat("short", 20),
          getCategories(),
        ]);
        setVideos(result.videos);
        setCursor(result.lastDoc);
  ...
  const loadMore = async () => {
    ...
    const result = user
      ? selectedCategory
        ? await getUserVideosByCategoryAndFormat(user.uid, selectedCategory, "short", 20, cursor)
        : await getUserVideosByFormat(user.uid, "short", 20, cursor)
      : selectedCategory
        ? await getVideosByCategoryAndFormat(selectedCategory, "short", 20, cursor)
        : await getVideosByFormat("short", 20, cursor);
